...
import { getAuthHeaders, getApiKey } from "../common/auth";
  ...
  return (
    <div style={{ position:"relative", margin:"8px 0", borderRadius:10, overflow:"hidden", border:"1px solid var(--border)" }}>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"5px 12px", background:"var(--bg-overlay)", borderBottom:"1px solid var(--border)" }}>
        ...
        <button className="btn btn-ghost btn-sm" style={{ fontSize:11 }} onClick={async()=>{ await navigator.clipboard.writeText(text); setCopied(true); setTimeout(()=>setCopied(false),1200); }}>
          {copied?"✓ Copied":"Copy"}
        </button>
      </div>
      <SyntaxHighlighter language={language} style={vscDarkPlus} customStyle={{ margin:0, background:"var(--bg-base)", fontSize:12, lineHeight:1.65 }}>{text}</SyntaxHighlighter>
    </div>
  );
}
  ...
  return (
    <div style={{ fontSize:14, lineHeight:1.75, color:"var(--text-primary)" }}>
      <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
        ...
        blockquote: ({children}) => <blockquote style={{ borderLeft:"3px solid var(--accent)", paddingLeft:12, margin:"8px 0", color:"var(--text-secondary)", background:"var(--bg-elevated)", borderRadius:"0 6px 6px 0", padding:"8px 12px" }}>{children}</blockquote>,
...
export function ChatPage() {
  ...
  const initModel      = sp.get("model") || "";
  const navigate       = useNavigate();
  const { toast }      = useToast();

  const [models, setModels]           = useState<string[]>([]);
  const [model, setModel]             = useState(initModel || "llama3.2:3b");
  ...
  useEffect(() => {
    api.get<any>("/models?pg_size=100").then(r => {
      const names = (r.items||[]).map((m:any)=>m.name);
      setModels(names.length ? names : MODELS_FALLBACK);
      if (!initModel && names.length) setModel(names[0]);
  ...
  useEffect(() => {
    if (isNearBottom.current) msgsEndRef.current?.scrollIntoView({ behavior:"smooth" });
  }, [messages]);
  ...
  const send = async () => {
    ...
    try {
      const startRes = await api.post<any>("/chat/start-auth", {
        model, messages: newMessages.map(m=>({ role:m.role, content:m.content })),
      ...
      const es = new EventSource(`${API_BASE}/chat/stream?request_id=${rid}&token=${apiKey}`);
      setEsRef(es);
      es.onmessage = evt => {
        try {
          ...
          if (data.event_type === "done" || data.event_type === "stopped") {
            es.close(); setStatus("idle"); setEsRef(null);
          }
        } catch {}
      };
      es.onerror = () => { es.close(); setStatus("idle"); setEsRef(null); toast("Stream error","error"); };
    } catch (e:any) {
      setStatus("idle");
  ...
  const copyMsg = async (content: string) => { await navigator.clipboard.writeText(content); toast("Copied","success"); };
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", overflow:"hidden" }}>
      ...
      {/* Messages */}
      <div ref={msgContainer} onScroll={handleScroll} style={{ flex:1, overflowY:"auto", padding:"20px 0", display:"flex", flexDirection:"column", position:"relative" }}>
        {messages.length===0 ? (
          <div className="empty-state" style={{ flex:1 }}>
            <div className="empty-state-icon" style={{ fontSize:52 }}>⌁</div>
            <div className="empty-state-title">Start a conversation</div>
            <div className="empty-state-desc">Ask anything. Markdown, code highlighting, and streaming are all included.</div>
          </div>
        ) : (
          <div style={{ maxWidth:820, margin:"0 auto", width:"100%", padding:"0 20px", display:"flex", flexDirection:"column", gap:16 }}>
            {messages.map((m, i) => (
              <div key={m.id} className="animate-slideup" style={{ display:"flex", flexDirection:"column", alignItems: m.role==="user"?"flex-end":"flex-start", maxWidth:"85%", alignSelf: m.role==="user"?"flex-end":"flex-start" }}>
                <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.08em", color: m.role==="user"?"var(--accent)":"var(--text-muted)", marginBottom:4, padding:"0 4px" }}>
                  {m.role==="user"?"You":"Assistant"}
                  {status==="streaming" && i===messages.length-1 && m.role==="assistant" && (
                    <span style={{ display:"inline-block", width:6, height:6, borderRadius:"50%", background:"var(--accent)", marginLeft:6, animation:"livepulse 1s ease-in-out infinite", verticalAlign:"middle" }} />
                  )}
                </div>
                <div style={{
                  padding:"12px 16px", borderRadius: m.role==="user"?"14px 14px 4px 14px":"14px 14px 14px 4px",
                  background: m.role==="user"?"var(--bg-overlay)":"var(--bg-surface)",
                  border: m.role==="user"?"1px solid var(--border-accent)":"1px solid var(--border-soft)",
                ...
                <div style={{ display:"flex", gap:4, marginTop:4, padding:"0 4px", opacity:0.6 }}>
                  <button className="btn btn-ghost btn-sm" style={{ fontSize:11 }} onClick={()=>copyMsg(m.content)}>Copy</button>
                  {m.token_count && <span style={{ fontSize:10, color:"var(--text-muted)", fontFamily:"var(--font-mono)", paddingTop:3 }}>{m.token_count} tokens</span>}
                </div>
              </div>
            ))}
            <div ref={msgsEndRef} />
      ...
      <div style={{ padding:"12px 20px", borderTop:"1px solid var(--border-soft)", background:"var(--bg-surface)", flexShrink:0 }}>
        <div style={{ maxWidth:820, margin:"0 auto" }}>
          <div style={{ display:"flex", gap:8, alignItems:"flex-end", background:"var(--bg-elevated)", border:"1px solid var(--border-strong)", borderRadius:14, padding:"8px 8px 8px 14px", transition:"border-color 150ms, box-shadow 150ms", boxShadow: input?"0 0 0 3px var(--accent-glow)":"none", borderColor: input?"var(--accent)":"var(--border-strong)" }}>
            <textarea
              ref={textareaRef}
              value={input}
          ...
          <div style={{ marginTop:5, textAlign:"center", fontSize:11, color:"var(--text-muted)" }}>
            {conversationId ? `Conversation saved · ${messages.length} messages` : "New conversation · will be saved after first message"}
          </div>
        </div>
      </div>
    </div>
