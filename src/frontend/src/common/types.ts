...
export interface Conversation { id: string; title: string; model_name: string; context_window: number; total_tokens: number; created_at: string; updated_at: string; is_archived: boolean; messages?: ChatMessage[]; }
export interface InstalledModel { name: string; size: number; size_gb: number; quantization?: string; family?: string; parameter_size?: string; modified_at?: string; }
...
export interface PullProgress { status: string; completed: number; total: number; percent: number; speed_mbps: number; eta_seconds: number|null; size_gb: number|null; }
